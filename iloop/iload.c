...
// ðàçáèâàåì ïðîãðàììó ÷åðåç ; òàê ÷òîáû áûëî ÷åðåç \n â list_prog 
int load_prog(char *progName, DblLinkedList * list_prog) {
	...
	if (file_prog != NULL) {
		...
		while ((ch = fgetc(file_prog)) != EOF) {
			if (!(ch == ' ' || ch == '\n' || ch == '\t')) {
				if (ch == ';') {
					buffer1[i] = '\0'; // ñòðîêà êîí÷èëàñü 
					int ex = 1;
					// íåñêîëüêî LOOP DO ïîäðÿä
					while (ex) {
...
// ðàçáèâàåì ïðîãðàììó ÷åðåç ; è , òàê ÷òîáû áûëî \n â list_var
